	...
	<view class="container">
		...
		<view v-if="!hasLogin || empty===true" class="empty">
			...
			<view v-if="hasLogin" class="empty-tips">
				<text style="margin-left: 80px;">空空如也</text>
				<view style="margin-top:20px">
					<button class="empty-button" @click="navToIndex">去逛逛</button>
				</view>
			</view>
			<view v-else class="empty-tips">
				<text style="margin-left: 80px;">暂未登录</text>
				<view style="margin-top:20px">
					<button class="empty-button" @click="navToLogin">去登陆</button>
				</view>
			</view>
		</view>
		<view v-else>
	...
	export default {
		...
		methods: {
			...
			handleDeleteCartItem(index) {
				...
				let row = list[index];
				let id = row.id;
				const method = "consumer/deleteProductFromShoppingCart?ids=" + index
				this.$api.request.removeCart(method,{}, res => {
					console.log(res);
					if (res.code === 200) {
			...
			clearCart() {
				uni.showModal({
					...
					success: (e) => {
						if (e.confirm) {
							this.$api.request.clearCart({}, res => {
								this.cartList = [];
							})
