			...
			>
				<swiper-item v-for="tabItem in tabBars" :key="tabItem.tagUuid">
					<scroll-view 
						...
						>
						<view @click="navToDetails(item.id)" v-for="(item, index) in tabItem.newsList " :key="index" class="news-item" >
							<text :class="['title', 'title'+item.articleType]">{{item.articleTitle}}</text>
							<view :class="['img-list', 'img-list'+item.articleType, item.articleType!='1' ? 'img-list-single': '']">
								...
								>
									<image class="img" :src="item.articlePic"></image>
							...
							<view :class="['bot', 'bot'+item.articleType]">
								<text class="author">药食同源大健康</text>
