				...
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData('add')">
					...
					<view v-for="(item,index) in orderList" :key="index" class="order-item">
						<view class="i-top b-b">
							<text class="time"
								@click="showOrderDetail(item.id)">{{item.createTime.slice(0,10)}} {{item.createTime.slice(11,19)}}</text>
							<text class="state" :style="{color: '#62cd6d'}">{{item.status | formatStatus}}</text>
						...
						<view class="goods-box-single" v-for="(orderItem, itemIndex) in item.orderItemList"
							...
							<view class="right">
								...
								x {{orderItem.productQuantity}}</text>
						...
						<view class="action-box b-t" v-if="item.status == 3">
							<button class="action-btn recom" @click="afterSale(item.id)">申请售后</button>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			loadData(type = 'refresh') {
				...
				this.$api.request.fetchOrderList(this.orderParam, res => {
					if (res.code === 200) {
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							...
							let method = 'consumer/deleteOrder?orderId=' + orderId;
							superThis.$api.request.delOrder(
								method, {}, res => {
									if (res.code === 200) {
										uni.hideLoading();
										superThis.loadData();
			...
			afterSale(orderId) {
				uni.navigateTo({
					url: `/pages/aftersale/index?orderId=${orderId}`
				})
			},
			//查看订单详情
		...
		.nav-item {
			...
			&.current {
				...
				&:after {
					...
					width: 44px;
					height: 0;
					border-bottom: 2px solid $button-color-light;
				}
			}
		}
	}
