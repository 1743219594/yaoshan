	...
	<view class="content">
		...
		<view class="order-item">
			<view class="i-top b-b">
				<text class="time">订单号: {{order.deliverySn}}</text>
			</view>

			<scroll-view v-if="order.orderProductDTOList&&order.orderProductDTOList.length > 1" class="goods-box"
				scroll-x>
				<view @click="navProductDetail(goodsItem)" v-for="(goodsItem, goodsIndex) in order.orderProductDTOList"
					:key="goodsIndex" class="goods-item">
					<image class="goods-img" :src="goodsItem.productImageUrl" mode="aspectFill"></image>
				</view>
			</scroll-view>
			<view v-if="order.orderProductDTOList&&order.orderProductDTOList.length === 1" class="goods-box-single"
				v-for="(goodsItem, goodsIndex) in order.orderProductDTOList" :key="goodsIndex">
				<image class="goods-img" :src="goodsItem.productImageUrl" mode="aspectFill"></image>
				<view class="right">
			...
			<view class="price-box">
				共
				<text class="num">{{order.orderItemList.length}}</text>
				件商品 实付款
		...
		<view class="row b-b">
			...
			<radio-group class="input" @change="radioChange">
				<label>
					<radio color="#09b980" value="1" :checked="afterSaleType=='1'" /><text>仅退款</text>
				</label>
				<!-- 待发货状态只需要退款即可 -->
				<label v-if="order.orderStatus != '1'">
					<radio color="#09b980" value="2" :checked="afterSaleType=='2'" /><text>退款退货</text>
				</label>
			</radio-group>
		</view>
		<view class="row">
			<text class="tit">退款金额</text>
			<input class="input" type="number" v-model="afterSaleAmount" placeholder="退款金额"
				placeholder-class="placeholder" />
		</view>
		<view class="aftersale-image">
			<text>上传凭证</text>
			<robby-image-upload v-model="imageUrlList" fileKeyName="files" :header="header" :formData="formData"
				:server-url="uploadUrl"></robby-image-upload>
		</view>
		<view class="aftersale-desc">
			<text>问题描述</text>
			<textarea v-model="afterSaleDescription" :maxlength="500" rows="5" placeholder="请您在此描述问题(最多500字)"
	...
	export default {
		...
		methods: {
			...
			inquiryOrder(orderNo) {
				let method = 'consumer/inquiryOrder?id=' + orderNo;
